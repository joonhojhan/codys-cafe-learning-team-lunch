...
describe('Coffee model', () => {
	...
	describe('column definitions and validations', () => {
		it('has a `name` and `ingredients`', async () => {
			...
			expect(puppaccino.ingredients).to.deep.equal([
				'espresso',
				'frothed milk',
				'love',
			]);
	...
	describe('instance method: getIngredients', () => {
		it('returns list of ingredients as a comma-delimited string', async () => {
			...
			expect(puppaccino.getIngredients()).to.equal(
				'espresso, frothed milk, love'
			);
			expect(frappeAllaPug.getIngredients()).to.equal(
				'espresso, ice, sugar, love'
			);
		});
	});
